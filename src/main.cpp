...
void setup() {
  ...
  if (!wifiManager.autoConnect("NotificationSetup")) {
    tftMain.fillScreen(COLOR_ERROR);
  ...
  delay(1000);
  tftMain.fillScreen(COLOR_BACKGROUND);
  tftMain.setTextColor(COLOR_SUCCESS);
  ...
  // init arrays
  for (int i = 0; i < MAX_NOTIFICATIONS; i++) notifications[i] = Notification();
...
void loop() {
  ...
  // Refresh reminder screen every minute for countdown updates
  ...
  if (millis() - wifiCheck > WIFI_CHECK_INTERVAL && WiFi.status() != WL_CONNECTED) {
    WiFi.reconnect();
    wifiCheck = millis();
  }
...
void drawAppIcon(int x, int y, String app) {
  ...
  if (app.indexOf("slack") >= 0)
    ...
    tftMain.fillCircle(x + 8, y + 8, 7, COLOR_WHATSAPP);
    tftMain.drawCircle(x + 8, y + 8, 7, COLOR_ICON_BORDER);
  } else if (app.indexOf("telegram") >= 0) {
    tftMain.fillCircle(x + 8, y + 8, 7, COLOR_TELEGRAM);
    tftMain.drawCircle(x + 8, y + 8, 7, COLOR_ICON_BORDER);
  } else {
    tftMain.fillRect(x, y, ICON_WIDTH, ICON_HEIGHT, COLOR_ICON_DEFAULT);
    tftMain.drawRect(x, y, ICON_WIDTH, ICON_HEIGHT, COLOR_ICON_BORDER);
  }
}
...
void updateClock() {
  ...
  tftMain.setTextSize(1);
  tftMain.setTextColor(COLOR_CLOCK);
  ...
  for (int i = 0; i < strlen(timeStr); i++) {
    if (timeStr[i] != previousTimeStr[i] || previousTimeStr[i] == '\0') {
      tftMain.fillRect(cursorX, y, maxCharWidth, charHeight, COLOR_BACKGROUND);
...
void refreshNotifications() {
  ...
  for (int i = 0; i < MAX_NOTIFICATIONS; i++) {
    ...
    if (notifications[i].message != "") {
      ...
      // Message starting from screen edge (x=5) for more chars
      tftMain.setTextColor(COLOR_NOTIF_MSG);
      String msg = notifications[i].message;
      if (msg.length() > NOTIF_MSG_MAX_CHARS)
        msg = msg.substring(0, NOTIF_MSG_MAX_CHARS) + "...";

      String msgLine1 = msg.substring(0, min(NOTIF_MSG_LINE_CHARS, (int)msg.length()));
      msgLine1.trim();
      tftMain.drawString(msgLine1, 5, y + 18);

      if (msg.length() > NOTIF_MSG_LINE_CHARS) {
        String msgLine2 = msg.substring(NOTIF_MSG_LINE_CHARS);
        msgLine2.trim();
        tftMain.drawString(msgLine2, 5, y + 36);
      }
    }
  }
}
...
void refreshRemindersDisplay() {
  ...
  for (int s = 0; s < count && shown < 5; s++) {
    ...
    // smaller icon (5px radius) - active red, future grey
    uint16_t iconColor = rm.triggered ? COLOR_REMINDER_ICON_ACTIVE : COLOR_REMINDER_ICON_INACTIVE;
    tftMain.fillCircle(10, y + 6, REMINDER_ICON_RADIUS, iconColor);
    tftMain.drawCircle(10, y + 6, REMINDER_ICON_RADIUS, COLOR_ICON_BORDER);

    // Line 1: [id] + due time
    tftMain.setTextColor(COLOR_REMINDER_DUE);
    ...
    tftMain.drawString(line1, 20, y);

    // Line 2: message starting from screen beginning (x=5) for more chars
    tftMain.setTextColor(rm.triggered ? COLOR_REMINDER_ACTIVE : COLOR_REMINDER_INACTIVE);
    String msg = rm.message;
    if (msg.length() > REMINDER_MSG_MAX_CHARS) msg = msg.substring(0, REMINDER_MSG_MAX_CHARS) + "...";
    tftMain.drawString(msg, 5, y + 18);
...
void triggerReminderVisuals(const Reminder &r) {
  ...
  if (r.color == COLOR_PRIORITY_HIGH) {
    r8 = 255;
    g8 = 0;
    b8 = 0;
  } else if (r.color == COLOR_PRIORITY_MEDIUM) {
    r8 = 255;
    g8 = 80;
    b8 = 0;
  } else {
...
void addStructuredNotification(String app, String from, String msg, uint16_t color) {
  ...
  if (color == COLOR_PRIORITY_HIGH) {
    rr = 255;
    gg = 0;
    bb = 0;
  } else if (color == COLOR_PRIORITY_MEDIUM) {
    rr = 255;
    gg = 80;
    bb = 0;
  }
...
void handleAddReminder(AsyncWebServerRequest *request) {
  ...
  Serial.printf("Added reminder id=%d msg=%s when=%ld limit=%d\n", reminders[idx].id, reminders[idx].message.c_str(), reminders[idx].when, reminders[idx].limitMinutes);
...
void handleListReminders(AsyncWebServerRequest *request) {
  ...
  for (int i = 0; i < MAX_REMINDERS; i++) {
    ...
    out += "\"priority\":" + String((r.color == COLOR_PRIORITY_HIGH) ? "\"high\"" : ((r.color == COLOR_PRIORITY_MEDIUM) ? "\"medium\"" : "\"normal\""));
    out += "}";
  }
  out += "]";
